...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let secrets = dotenv::vars().collect::<std::collections::HashMap<String, String>>();

    let db_connection = secrets.get("DB_CONNECTION");
    let redis_connection = secrets.get("REDIS_CONNECTION");
    let api_url = secrets.get("API_URL").to_owned().unwrap();

    // Create RedisState asynchronously
    let redis_state = RedisState::new(redis_connection.as_deref().unwrap())
        .await
        .expect("Failed to create Redis connection");

    let (pg_client, connection) = tokio_postgres::connect(db_connection.as_deref().unwrap(), NoTls)
        .await
